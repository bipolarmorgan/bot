...
module.exports = class extends BaseCommand {
    ...
    async run(client, message, args, settings) {
        ...
        if (!target) return message.channel.send('Sorry, you can\'t invite invalid users to this ticket');
        if (target.id === message.author.id) return message.channel.send('Sorry, you cannot invite yourself to this ticket');
        const db = new GuildTickets(message.guild.id);
        const ticket = await db.find(message.author.id);
        if (!ticket || message.channel.id !== ticket.channel) return message.channel.send('Oi, you can\'t invite users to this ticket because it\'s not a ticket :P');
        ...
        if (message.author.id === ticket.id ||
            ...
            await message.channel.createOverwrite(target, {
                ...
                console.error(e);
                return message.channel.send('Sorry, something went wrong inviting the user');
            });
        } else return message.channel.send('Sorry, you don\'t have the permission to invite users to this ticket');
    }
}