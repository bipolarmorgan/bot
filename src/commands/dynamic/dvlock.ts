...
export default class dvlock extends Command {
    constructor() {
        super({
            config: {
                name: 'dvlock',
                description: 'Lock anyone from connecting to your dynamic voice channel!',
                permission: 'User',
            },
            options: {
                aliases: [],
    ...
    async run(client: Client, message: Message, args: string[], guildSettings: Guild) {
        ...
        try {
            ...
            message.channel.send('Your voice channel has been locked!');
        } catch (error) {
            message.channel.send(`Oh oh, something went wrong setting the locking the voice channel\n${error.message}`);
        }
    }
}